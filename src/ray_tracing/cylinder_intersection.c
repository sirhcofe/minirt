...
/*   Updated: 2023/10/24 18:50:54 by chenlee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_coord t_ray_ori, t_cy *cylinder)
{
	...
	intersection[0] = MAX_INT;
	intersection[1] = MAX_INT;
	coeff[0] = pow(t_ray_vec.x, 2) + pow(t_ray_vec.y, 2);
	coeff[1] = 2 * (t_ray_vec.x * t_ray_ori.x + t_ray_vec.y * t_ray_ori.y);
	coeff[2] = pow(t_ray_ori.x, 2) + pow(t_ray_ori.y, 2) - pow(cylinder->dia / 2, 2);
...
double	cylinder_intersection(t_coord ray_vec, t_coord ray_ori, t_cy *cylinder)
{
	t_coord	trans_ray_vec;
	t_coord	trans_ray_ori;
	double	intersection[2];

	trans_ray_vec = transform_to_cylinder_space(ray_vec, cylinder, 1);
	trans_ray_ori = transform_to_cylinder_space(ray_ori, cylinder, 2);
	if (found_intersection(intersection, trans_ray_vec, trans_ray_ori, cylinder))
		return 1;
	else
		return 0;
}
